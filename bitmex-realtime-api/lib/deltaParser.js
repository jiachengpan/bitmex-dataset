...
module.exports = {
  ...
  _delete(context, key, data, keys, is_orderbook) {
    return removeFromStore.apply(null, arguments);
  },

  _insert(context, key, data, keys, is_orderbook) {
    return insertIntoStore.apply(null, arguments);
  },

  _partial(tableName, symbol, client, data, is_orderbook) {
  ...
  _update(context, key, data, keys, is_orderbook) {
 ...
 */
function insertIntoStore(context, key, newData, is_orderbook) {
  ...
  let delta = [];
  if (is_orderbook) {
    delta = _.map(storeData, function (item) {
      return {'side': item.side, 'd_size': item.size, 'price': item.price};
    });
  }

  return [replaceStore(context, key, storeData), delta];
}
 ...
 * @param  {Array}  keys    Key names that uniquely identify a datum.
 */
function updateStore(context, key, newData, keys, is_orderbook) {
  const store = context[key] || [];
  ...
  for (let i = 0; i < newData.length; i++) {
    ...
    if (itemToUpdate) {
      if (is_orderbook) {
        delta.push({'side': itemToUpdate.side, 'd_size': newDatum.size - itemToUpdate.size, 'price': itemToUpdate.price});
      }
 ...
 * @param  {Array}  keys    Key names that uniquely identify a datum.
 */
function removeFromStore(context, key, newData, keys, is_orderbook) {
  const store = context[key] || [];
  ...
  for (let i = 0; i < newData.length; i++) {
    ...
    if (is_orderbook) {
      delta.push({'side': itemToRemove.side, 'd_size': -itemToRemove.size, 'price': itemToRemove.price});
    }
  }
